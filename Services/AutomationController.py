...
class AutomationController(Thread):
    ...
    def __initializeDataStructures(config: Settings, physicalDeviceNamesList: list, sb: IMessageBus, db: DatabaseContext) -> tuple:
        ...
        locState.Device = [Thermometer(devices[key].Name, 0) for key in devices]
    ...
    def run(self) -> None:
        ...
        while not self.isStopRequested():
            ...
            if delta.total_seconds() >= min([4, (localSettings.DataSaveInterval >> 1)]) and future is None:
            ...
            if self._isAutoMode and delta.total_seconds() >= localSettings.ControlInterval and actualTime.hour >= localSettings.DayStart and actualTime.hour < localSettings.DayStop and not (lowerTemp is None or higherTemp is None or ambientTemp is None):
                ...
                if localSettings.HigherTempSensor in self._devices and localSettings.LowerTempSensor in self._devices:
                    ...
                    if samplesCount > 0:
                        pts1: dict = self._databaseContext.getPoints(dayIndx, idHigher, localSettings.MaxStateTime * 2)
                        pts2: dict = self._databaseContext.getPoints(dayIndx, idLower, localSettings.MaxStateTime * 2)
                        data: list = list()
                        for key in pts1:
                            if key in pts2:
    ...
    @staticmethod 
    def __filterTemperaturesAndAddOffsets(measurements: dict, measurementFilter: IMeasurementFilter, localSettings: Settings, logger: logging) -> dict:
        for label in measurements:
            ...
            if temperature is None:
                ...
                measurementFilter.updateMeasurements(label, temperature, localSettings.MeasurementSamplesCount)
                filteredTemp = measurementFilter.get(label)
    ...
    def __updateLocalStatusTemperatures(measurements: dict, localStatus: ObjectState, logger: logging) -> ObjectState:
        ...
        for label in measurements:
            ...
            if filteredTemp != 999 and filteredTemp is not None:
    ...
    @staticmethod
    def __addOffsets(value: float, deviceName: str, config: Settings) -> float:
        if value is None:
            return None
